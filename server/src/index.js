...
class GameSession {
    ...
    update() {
        ...
        if (this.servingState.isServing) {
            this.ball.x = this.servingState.servingPlayer === 1 ? COURT_WIDTH / 4 : COURT_WIDTH * 3/4;
    ...
    checkPlayerBallCollisions() {
        [this.player1, this.player2].forEach((player, index) => {
            ...
            if (distance < BALL_RADIUS + PLAYER_WIDTH / 2) {
                ...
                const speed = Math.sqrt(this.ball.velocity.x ** 2 + this.ball.velocity.y ** 2) || 10;
    ...
    endGame() {
        ...
        const winner = this.player1.score > this.player2.score ? 1 : 2;
